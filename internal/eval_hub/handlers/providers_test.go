...
func TestHandleListProviders_ReturnsSystemProviders(t *testing.T) {
	...
	h := handlers.New(storage, validation.NewValidator(), &fakeRuntime{}, nil, nil, nil, nil)
...
func TestHandleListProviders_AppliesPaginationWhenLimitLessThanSystemProviders(t *testing.T) {
	...
	logger := slog.New(slog.NewTextHandler(io.Discard, nil))
	h := handlers.New(&fakeStorage{providerConfigs: providerConfigs}, validation.NewValidator(), &fakeRuntime{}, nil, nil, nil, nil)
...
func TestHandleListProviders_Returns400WhenInvalidLimit(t *testing.T) {
	logger := slog.New(slog.NewTextHandler(io.Discard, nil))
	h := handlers.New(&fakeStorage{}, validation.NewValidator(), &fakeRuntime{}, nil, nil, nil, nil)
